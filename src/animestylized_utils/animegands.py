...
class AnimeGANDataModule(pl.LightningDataModule):
        ...
        self.val_transform = transforms.Compose(
            [
                ...
                transforms.Normalize(
                    mean=(0.5, 0.5, 0.5), std=(0.5, 0.5, 0.5) if normalize else idenity
                ),
            ]
        )
    ...
    def setup(self, stage=None):
        if stage == "fit":
            ...
            train_real = ImageFolder(
                trian_root.as_posix(), transform=self.train_real_transform
            )
            train_anime = ImageFolder(
                anime_root.as_posix(), transform=self.train_anime_transform
            )
            train_anime_gray = ImageFolder(
                anime_root.as_posix(), transform=self.train_gray_transform
            )
            train_anime = TensorDataset(train_anime, train_anime_gray)
            train_smooth_gray = ImageFolder(
                smooth_root.as_posix(), transform=self.train_gray_transform
            )
            self.ds_train = MergeDataset(train_real, train_anime, train_smooth_gray)

            val_root = self.root / "test/test_photo"
            self.ds_val = ImageFolder(val_root.as_posix(), transform=self.val_transform)
        else:
            val_root = self.root / "test/test_photo"
            self.ds_val = ImageFolder(val_root.as_posix(), transform=self.val_transform)
    ...
    def val_dataloader(self):
        return DataLoader(self.ds_val, shuffle=True, batch_size=4, num_workers=4)
