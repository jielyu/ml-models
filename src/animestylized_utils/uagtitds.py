...
class UagtitGanDataSet(pl.LightningDataModule):
    ...
    def setup(self, stage=None):
        if stage == "fit":
            self.trainA = ImageFolder(
                self.root + "/trainA" + self.A_suffix, transform=self.train_transform
            )
            self.trainB = ImageFolder(
                self.root + "/trainB" + self.B_suffix, transform=self.train_transform
            )
            self.ds_train = MergeDataset(self.trainA, self.trainB)
            self.testA = ImageFolder(
                self.root + "/testA" + self.A_suffix, transform=self.test_transform
            )
            self.testB = ImageFolder(
                self.root + "/testB" + self.B_suffix, transform=self.test_transform
            )
    ...
    def val_dataloader(self):
        return DataLoader(
            self.ds_test, sampler=MultiSequentialSampler(self.ds_test), batch_size=4
        )
