...
class YOLOXHead(nn.Module):
    ...
    def forward(self, xin):
        ...
        for k, x in enumerate(xin):
            ...
            output = torch.cat(
                [reg_output, torch.sigmoid(obj_output), torch.sigmoid(cls_output)], 1
            )
            ...
            if self.training is True:
                ...
                expanded_strides.append(
                    torch.zeros(1, grid.shape[1]).fill_(stride_this_level).type_as(x)
                )
...
class YoloxLoss(nn.Module):
    ...
    def forward(
        ...
        assert (
            len(origin_reg) == len(origin_obj)
            and len(origin_obj) == len(origin_cls)
            and len(origin_cls) == len(origin_grid)
            and len(origin_grid) > 0
        )
        # 输入数据格式预处理
        ...
        if self.use_l1:
            loss_l1 = (
                self.l1_loss(pred_output.view(-1, 4)[fg_masks], l1_targets)
            ).sum() / num_fg
        # 返回混合损失
        reg_weight = 5.0
    ...
    def get_assignments(
        ...
        # 计算分类损失
        gt_cls_per_image = (
            F.one_hot(gt_classes.to(torch.int64), cls_preds.shape[-1])
            .float()
            .unsqueeze(1)
...
class Exp:
    ...
    def resume_model(cls, load_dir, model, optimizer, device="cuda"):
        ...
        if lastest_model_path is not None and os.path.isfile(lastest_model_path):
            # 载入模型文件
            ...
            if "model" in ckpt and "optimizer" in ckpt and "start_epoch" in ckpt:
                ...
                logger.info(
                    "resume model at epoch {} from {} succ".format(
    ...
    def save_model(save_dir, model, optimizer, epoch, loss):
        ...
        # 保存到文件
        filename = os.path.join(save_dir, "yolox_epoch_{}_ckpt.pth".format(epoch))
    ...
    def train(self, is_gpu=False, no_aug=False):
        """用于训练模型"""
        logger.info(
            "starting train model in {} mode ...".format("CUDA" if is_gpu else "CPU")
        )
        # 创建模型
        ...
        start_epoch, start_model_file = self.resume_model(
        ...
        for idx_epoch in range(start_epoch, self.max_epoch):
            for idx_iter, batch in enumerate(train_loader):
                ...
                # 更新学习率
                lr = lr_scheduler.update_lr(idx_iter + idx_epoch * num_train_iters + 1)
            ...
            model_path = self.save_model(
                self.output_dir, model, optimizer, idx_epoch, loss_value
            )
    ...
    def demo(self, img_file, model_file, conf_thre=0.25, nms_thre=0.45, is_gpu=False):
        """用于测试检测效果的demo"""
        logger.info(
            "starting load model in {} mode ...".format("CUDA" if is_gpu else "CPU")
        )
        device = "cuda" if is_gpu is True else "cpu"
