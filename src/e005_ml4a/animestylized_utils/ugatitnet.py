...
class SoftAdaLIN(nn.Module):
    ...
    def forward(self, x, content_features, style_features):
        ...
        w_gamma, w_beta = self.w_gamma.expand(x.shape[0], -1), self.w_beta.expand(
            x.shape[0], -1
        )
        soft_gamma = (1.0 - w_gamma) * style_gamma + w_gamma * content_gamma
        soft_beta = (1.0 - w_beta) * style_beta + w_beta * content_beta
