...
class WhiteBoxGANDataModule(pl.LightningDataModule):
    ...
    def setup(self, stage=None):
        scenery_cartoon = ImageFolder(
            (self.root / f"scenery_cartoon/{self.scene_style}").as_posix(),
            transform=self.transform,
        )
        scenery_photo = ImageFolder(
            (self.root / "scenery_photo").as_posix(), transform=self.transform
        )
        n_scenery = len(scenery_photo)
        scenery_photo_train, scenery_photo_val = random_split(
            scenery_photo, [int(n_scenery * 0.9), n_scenery - int(n_scenery * 0.9)]
        )

        scenery_ds = MergeDataset(scenery_cartoon, scenery_photo_train)

        face_cartoon = ImageFolder(
            (self.root / f"face_cartoon/{self.face_style}").as_posix(),
            transform=self.transform,
        )
        face_photo = ImageFolder(
            (self.root / "face_photo").as_posix(), transform=self.transform
        )
        n_face = len(face_photo)
        face_photo_train, face_photo_val = random_split(
            face_photo, [int(n_face * 0.9), n_face - int(n_face * 0.9)]
        )
